...
function Dynamsoft_OnReady() {
    ...
    if (DWObject) {
        if (DWObject.ErrorCode == 0) {
			...
			if(DWObject.__strProductName)
			{
				var serverName = DWObject.__strProductName.toLowerCase();
				if(serverName.indexOf('trial')<0)
				{
					var oldLink = location.href.toLowerCase(), curPage;
					curPage = oldLink.substring(oldLink.lastIndexOf('/')+1);
					
					location.href='/DWTFull/' + curPage;
					return;
				}
			}
