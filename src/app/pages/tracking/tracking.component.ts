...
export class TrackingComponent implements OnInit {
  ...
  transactionType: any = [];
  DateCreated;
  switchTabs: string = 'trackingList';
  receiverUserCol: any;
  ...
  getAllInvTracking() {
    this.serviceProider.getAllInvTracking().subscribe(res => {
      ...
      for (var i = 0; i < res.length; i++) {
        var findVal = res[i].CreatedByUserId + ' ' + this.parseDate(res[i].DateCreated);
        if (!temp_var1.includes(findVal)) {
          ...
          var index = temp_var1.indexOf(findVal);
          var tmp = temp_var2[index];
  ...
  gettransactionType() {
    let data = JSON.parse(sessionStorage.getItem("TaxonomyItemDetail"));
  ...
  saveTracking(orderData, scannedItemsList, receiverData) {
    ...
    for (var i = 0; i < scannedItemsList.length; i++) {
      let data = {
  ...
  addItemToTracking(event) {
    ...
    var findObj = this.scannedItemsList.find(val => val.ItemCode == event.data.ItemCode && val.BatchCode == event.data.BatchCode);
    if (findObj !== undefined) {
      // console.log('finded')
      for (var i in this.scannedItemsList) {
        if (this.scannedItemsList[i].ItemCode == event.data.ItemCode && this.scannedItemsList[i].BatchCode == event.data.BatchCode) {
          var init_qnty = (isNaN(this.scannedItemsList[i].Quantity) ? 1 : this.scannedItemsList[i].Quantity);
