...
export class DwtComponent implements OnInit {
  imageType = 'png';
  constructor(private authStore: AuthenticationStore, private newFileStoreService: FileStorageService, private formService: FormService, private toaster:ToastrService){

  }
  ngOnInit() {
    // Dynamsoft.WebTwainEnv.ProductKey = 't0068MgAAAEG5msl0gsy3LaC1YWHDdbtRvzoAWgPHVycodXPBJ08eTShT2xOTPm8J51fIAZ7Jw6O8sgHrgjE1sA6qYX3u+xM=';
  ...
  btnUpload_onclick(): void {
    ...
    let uploadfilename = fileName + "." + this.imageType;
    ...
    let blob = this.DWObject.ConvertToBlob (
      ...
      (result) => {
        ...
        var formData = new FormData();
         ...
         this.newFileStoreService.addFile("Scanned document" + Date.now(),formData).subscribe(event => {
...
console.log("blob", blob)
  ...
  this.formService.addPatientDoc(newform).subscribe(event => {        
      this.showToaster('Scanned document Saved successfully!')
  });
}
