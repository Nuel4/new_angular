...
createChart(){
  $(() => {this.LineChart1 = new Chart('bmiChart', {
    type: 'line',
    ...
    options: {
      ...
      title: {
        display: true,
        text: 'BMI chart with 2 to 20 years percentile data'
      },
      scales: {
        xAxes: [{
          scaleLabel: {
...
saveChart(chartName) {
  ...
  var dataUrl = canvas.toDataURL(); //attempt to save base64 string to server using this var  
    ...
    windowContent += '<html>'
    windowContent += '<head><title>BMI Chart</title></head>';
    windowContent += '<body>'
