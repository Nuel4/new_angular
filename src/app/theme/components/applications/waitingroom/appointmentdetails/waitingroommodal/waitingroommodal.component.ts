...
import { BillersNoteComponent } from "../../billers-note/billers-note.component";
import * as moment from "moment";
import { DocumentActionsComponent } from "../../../../../../../app/pages/chart/documents/document-actions/document-actions.component";
import { ChargeslipsComponent } from '../../../../../../../app/chargeslips/chargeslips.component';
@Component({
  selector: "app-waitingroommodal",
  templateUrl: "./waitingroommodal.component.html",
  styleUrls: ["./waitingroommodal.component.scss"],
  encapsulation: ViewEncapsulation.None
})
export class WaitingroommodalComponent implements OnInit {
  @Input() openFormsModal: boolean;
  @Input() openDocumentModal: boolean;
  @Input() openCopayModal: boolean;
  @Input() openAccountSummaryModal: boolean;
  @Input() openCancelApptModal: boolean;
  @Input() apptId: any;
  ...
  tertiary_insurance: any;
  openBillersModal: boolean = false;
  openPastModal: boolean = false;
  ...
  ngOnInit() {
    this.cols = [
      ...
      { field: "", header: "Document Name", filter: true },
      { field: "displayDate", header: "File Type", filter: true },
    ...
    this.co_pay = this.openCopayModal ? this.patientData.copayamount : '';

    // this.account_cols = [
    //   { field: 'patientname', header: 'Days  0-to-30'},
    //   { field: "username", header: "31-to-60"},
    //   { field: "appointmentstarttime", header: "61-to-90"},
    //   { field: 'appointmenttype', header: '91-to-120'},
    //   { field: "eligibility", header: "121-to-150" },
    //   { field: "eligibility", header: "150+" },
    // ]
    this.patient_name =
      JSON.parse(sessionStorage.getItem("PatientDetail")) &&
      JSON.parse(sessionStorage.getItem("PatientDetail")).FirstName +
        JSON.parse(sessionStorage.getItem("PatientDetail")).LastName;
    if (this.openCancelApptModal) {
      this.getReasonsForCancel();
    }
    if (this.openAccountSummaryModal) {
      this.getAccountSummary();
    }
    if (this.openDocumentModal) {
  ...
  cancelAppt() {
    this.appoinmentService
      .CancelAppointment(
        ...
        this.authStore.UserDetail.UserId
      )
      .subscribe(result => {
  ...
  getAccountSummary() {
    ...
    if (this.patientData) {
      this.waitingRoomService
        .getAccountSummary({ patientId: this.patientData.patientId })
        .subscribe((results:any) => {
          this.account_summary = results.length > 0 ? results[0] : {};
          let param = {
            // 35871
            patientID: this.patientData.patientId
            // patientID: 35871
          };
          this._patientmanagementService
            .GetPatientInsurance(param)
            .subscribe(res => {
              if (typeof res !== "undefined" && res.length > 0) {
                this.patientInsurance = res;
                res.forEach(item => {
                  if (item.order === 1) {
                    this.primary_insurance = item.insurancename;
                  }
                  if (item.order === 2) {
                    this.secondary_insurance = item.insurancename;
                  }
                  if (item.order === 3) {
                    this.tertiary_insurance = item.insurancename;
                  }
                });
              }
            });
          ...
          this.waitingRoomService
            .getPatientPayments({ patientId: this.patientData.patientId })
            .subscribe((results: any) => {
              this.patientPaymentDetails =
                results[0].PaymentAmount -
                results[0].TotalAmountAppliedToCharges;
            });
        });
    } else {
  ...
  openModal(modal_type) {
    ...
    if (modal_type === "Past_Statements") {
      ...
      modalRef.componentInstance.name = "true";
      modalRef.componentInstance.openPastModal = true;
      modalRef.componentInstance.patientData = this.patientData;
    }
  }
  ...
  addCopay() {
    // let payload = {
    // "AppointmentId": this.patientData.appointmentid,

    this.appoinmentService
      .GetAppointmentById(this.patientData.appointmentid)
      .subscribe((results: any) => {
        let payload = results[0];
        payload.Deductible = parseFloat(this.deductible);
        payload.CoInsurancePatientAmount = parseFloat(this.co_Insurance);
        payload.CoPayAmount = parseFloat(this.co_pay);
        payload.PaymentMethod = this.selectedPayment.paymentName;
        payload.CheckNumber = parseInt(this.check_number);
        payload.LastUpdatedByUserId = this.authStore.UserDetail.UserId;
        payload.DateLastUpdated = moment(new Date()).format("YYYY-MM-DD");
        this.waitingRoomService.addCopay(payload).subscribe(result => {
          this.showSuccess("added copay successfully");
          this.loadEvent.emit(true);
  ...
  getDmsCategory() {
    this.waitingRoomService.getDmsCategory().subscribe((results: any) => {
      this.dmsCategories = results;
      //   this.getDocumentList();
    });
  }
  ...
  getDocumentList(pageNo?) {
    ...
    this.waitingRoomService
      ...
      .subscribe((results: any) => {
        this.documentsLists = results;
      });
  }

  paginate(event) {
    this.getDocumentList(event.index);
  }
  ...
  openPdfPrint() {
    ...
    modalRef.componentInstance.patientData = this.patientData;
    modalRef.componentInstance.Component = "WaitingRoomPrint"
  //   var divElements =
  //     `<div>
  //   <div>
  //     <label>Unique Number: ` +
...
//                 <div class="t m0 x1 hc y38 ff1 fs4 fc0 sc0 ls0 ws0"> 888 test</div>
//             </div>
...
//                 <div class="t m0 x1 h3 y3c ff1 fs0 fc0 sc0 ls0 ws0"> /</div>
//             </div>
